...
# Heartbeat  ---------------------------------------------------
    ...
    def onHeartbeat(self):
        ...
        if self.NukiLastCallBack + timedelta(minutes=self.Poll) <= now  :
            ...
            if checkAPI :
                Domoticz.Debug("Updating DZ Widget values...")
                self.SMdoorstate = checkAPI["doorsensorState"]
                self.SMlockstate = checkAPI["state"]
                self.SMbatt = checkAPI["batteryChargeState"]
                Domoticz.Debug("Plugins values : Door state = {}, Lock state = {}, and Batt. = {} %".format(self.SMdoorstate, self.SMlockstate,self.SMbatt))
                # Updating DZ Widget according to Nuki SM State response
                # Door State
                if Parameters["Mode1"] == "":
                    Domoticz.Debug("Door sensor used : Nuki Smart Lock one")
                    if self.SMdoorstate == "None" :
                        Domoticz.Error("Nuki SM have not Door sensor, please use external one...")
                    else :
                        if self.SMdoorstate == 2:
                            Devices[1].Update(nValue=0, sValue="Closed")
                        elif self.SMdoorstate == 3:
                            Devices[1].Update(nValue=1, sValue="Open")
                        else:
                            Domoticz.Error("Nuki SM API response : DOOR state unknown")
                            Devices[1].Update(nValue=1, sValue="Open")
                else :
                    Domoticz.Debug("Door sensor used : External PS")
                    self.PSactif = False
                    noerror = True
                    ...
                    if devicesAPI:
                        for device in devicesAPI["result"]:  # parse the perimetral sensors (PS) devices
                            idx = int(device["idx"]
                            if idx in self.DoorContact: # this is one of our PS
                                if "Status" in device:
                                    Domoticz.Debug("Verif : PS Sensor idx {}, '{}' currently is '{}'".format(idx, device["Name"],device["Status"]))
                ...
                Domoticz.Debug("... DZ Widget values updated")
    ...
    try:
        ...
        if response.status == 200:
            ...
            if resultJson["success"] != True:
                ...
                Domoticz.Debug("Nuki API reply : Success = {}, Door state = {}, Lock state = {}, and Batt. = {} %".format(resultJson["success"], resultJson["doorsensorState"], resultJson["state"], resultJson["batteryChargeState"]))
        else:
            Domoticz.Error("Nuki SM API: http error = {}".format(response.status))
    except:
        Domoticz.Log("Error calling '{}'".format(url))
    return resultJson
