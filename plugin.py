    ...
    def onHeartbeat(self):
        ...
        if self.NukiLastCallBack + timedelta(minutes=self.Poll) <= now  :
            ...
            if checkAPI :
                ...
                for doorsensorState in checkAPI["doorsensorState"]:
                    self.SMdoorstate = int("doorsensorState")
                #resultJson = json.loads(response.read().decode('utf-8'))
                #self.SMdoorstate = resultJson["doorsensorState"]
                #self.SMlockstate = resultJson["state"]
                #self.SMbatt = resultJson["batteryChargeState"]
